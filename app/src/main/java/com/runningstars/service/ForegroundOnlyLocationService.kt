 ...
 */
package com.runningstars.service
...
import com.runningstars.MapsActivity
...
class ForegroundOnlyLocationService : Service() {
    ...
    companion object {
        ...
        private const val PACKAGE_NAME = "com.runningstars"
        ...
        private const val NOTIFICATION_ID_STOP = 87654321

        private const val NOTIFICATION_CHANNEL_ID = "running_stars_channel_01"
    }
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        ...
        if (cancelLocationTrackingFromNotification) {
            ...
            stopSelf()
            notificationManager.notify(NOTIFICATION_ID_STOP, generateNotificationStopped())
        }
    ...
    private fun getLastLocation() {
        fusedLocationProviderClient.lastLocation.addOnCompleteListener { task ->
            if (task.isSuccessful && task.result != null) {
                broadcastLocation(task.result as Location)
            } else {
    ...
    private fun generateNotification(location: Location?): Notification {
        ...
        // 0. Get data
        val mainNotificationText = start?.let { String.format("%.0f m in %s", distance, DateTimeTool.counterTime(it, LocalDateTime.now())) }
...
//            )
            .addAction(
                ...
                getString(R.string.stop_location_updates_notif_text),
     ...
     * Generates a BIG_TEXT_STYLE Notification that represent latest location.
     */
    private fun generateNotificationStopped(): Notification {
        Log.d(TAG, "generateNotificationStopped()")
        ...
        // 0. Get data
        val mainNotificationText = start?.let { String.format("Stopped ! %.0f m in %s", distance, DateTimeTool.counterTime(it, LocalDateTime.now())) }
            ?: String.format("%.0f m", distance)
        val titleText = getString(R.string.app_name)
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult?) {
                ...
                if (locationResult?.lastLocation != null) {
                    ...
                    distance = currentLocation?.let { distance + location.distanceTo(it)  } ?: 0.0F
